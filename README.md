...
Lambda Warm-up Schedule: To prevent early accuracy collapse, the sparsity penalty (λ) is linearly ramped up over the initial 3 epochs. This allows the model to learn foundational features before the pruning pressure becomes significant.
...
$$Total Loss = L_{CrossEntropy} + \lambda \times \sum_{i,j} |\sigma(s_{i,j})|$$
Where:
...
The L1 penalty (sum of absolute values) is used because its constant gradient ($\pm 1$) effectively drives less significant gate values toward zero, inducing sparsity.
...
1  * 10^{-4}$	50.79%	98.80%	Optimal Balance: High sparsity with maintained performance.
1  * 10^{-3}$	46.83%	99.93%	Aggressive pruning with moderate accuracy trade-off.
1  * 10^{-2}$	42.31%	100.00%	Extreme compression; model uses minimal active weights.
